...
UPDATE "difference-of-squares" SET result =(
    CASE "property"
        WHEN 'squareOfSum' THEN pow((1 + number)*number/2, 2)
        WHEN 'sumOfSquares' THEN  (number * (number + 1) * (2 * number + 1) / 6)
        WHEN 'differenceOfSquares' THEN  pow((1 + number)*number/2, 2) - (number * (number + 1) * (2 * number + 1) / 6)
        ELSE 0
    END
);